...
class StorageFile:
  ...
  def readPageHeader(self, pageId):
  	...
  	page_packed = self.file.read( self.pageSize() )
  	return self.pageClass().headerClass.unpack(page_packed)
  ...
  def writePage(self, page):
    ...
    if not self.validPageId(pId):
      # update the pageNum and check if there is available space in that page
      self.pageNum += 1
  ...
  def insertTuple(self, tupleData):
    ...
    page = self.freePages[first_ava_pageId]
  ...
  def deleteTuple(self, tupleId):
    ...
    if self.freePages.has_key(pageId):
      ...
      return
    pageBuffer = bytearray(self.pageSize())
    page_retrieved = self.readPage(pageId, pageBuffer)
    page_retrieved.deleteTuple(tupleId)
    # check if this page is free now
    if not self.freePages.has_key(pageId):
      if page_retrieved.hasFreeTuple():
        self.freePages[pageId] = page_retrieved
    self.writePage(page_retrieved)
